...
def chat_with_bot(request: ChatRequest, db: Session = Depends(get_db)):
    try:
        # 1. RAG पाइपलाइन से जवाब जनरेट करें
        answer = generate_rag_response(bot_id=request.bot_id, question=request.question)
        
        # 2. यूज़र का मैसेज सेव करें (Unique UUID ID के साथ)
        ...
        # 3. बॉट का रिस्पॉन्स सेव करें (Unique UUID ID के साथ)
        bot_msg = ChatMessage(
            id=f"msg_{uuid.uuid4().hex[:12]}",
            bot_id=request.bot_id,
            sender="bot",
...
def get_chat_history(bot_id: str, db: Session = Depends(get_db)):
    messages = db.query(ChatMessage).filter(ChatMessage.bot_id == bot_id).order_by(ChatMessage.created_at.asc()).all()
    return [
        {
            "id": msg.id,
            "sender": msg.sender,
